...
<section class="hero-section">
    <div class="container hero-grid">
        <div class="hero-copy">
            ...
            <div class="hero-trust"><span><?= icon('shield-check') ?> Safe coding workspaces</span><span><?= icon('terminal') ?><?= number_format($stats['languages']) ?> programming languages</span><span><?= icon('school') ?> Built for guided learning</span></div>
        </div>
        <div class="hero-product" aria-label="CodeMwana learning preview">
            <div class="browser-frame">
                <div class="browser-bar"><span></span><span></span><span></span><small><?= e(setting('site_name', 'CodeMwana')) ?> learning workspace</small></div>
                <div class="preview-shell">
                    ...
                    <div class="preview-main">
                        <div class="preview-top"><div><small>Learning preview</small><strong><?= e($previewCourse['title'] ?? 'Learning paths are ready') ?></strong></div><span class="avatar small"><?= e($previewCourse ? initials($previewCourse['title']) : 'CM') ?></span></div>
                        <div class="preview-progress"><div><span>Featured learning path</span><strong><?= e($previewCourse['title'] ?? setting('site_name', 'CodeMwana')) ?></strong><small><?= $previewCourse ? (int) $previewCourse['lesson_count'] . ' guided lessons' : 'Learning content will appear here' ?></small></div><div class="progress-ring"><b><?= $previewCourse ? (int) $previewCourse['lesson_count'] : 0 ?></b></div></div>
                        <div class="preview-cards"><article><span><?= icon($previewLesson['icon'] ?? 'terminal') ?></span><strong><?= e($previewLesson['title'] ?? 'Begin your first activity') ?></strong><small><?= $previewLesson ? (int) $previewLesson['duration_minutes'] . ' minutes' : 'Guided practice' ?></small></article><article><span><?= icon('route') ?></span><strong><?= e($previewCourse['level'] ?? 'Beginner') ?></strong><small><?= e($previewCourse['audience'] ?? 'Structured learning') ?></small></article></div>
            ...
            <div class="floating-card floating-one"><?= icon('blocks') ?><span><strong><?= number_format($stats['lessons']) ?> published lessons</strong><small>Ready for guided learning</small></span></div>
            <div class="floating-card floating-two"><?= icon('folder-code') ?><span><strong><?= number_format($stats['projects']) ?> learner projects</strong><small>Saved for continued practice</small></span></div>
        </div>
    </div>
</section>
...
<section class="section" id="learning">
    <div class="container">
        <div class="section-heading centered"><span class="eyebrow">Structured learning</span><h2>Start with thinking. Progress to creating.</h2><p>Each path has clear outcomes, ordered lessons, practical challenges and helpful assessment questions.</p></div>
...
<section class="section section-soft" id="experience">
    <div class="container">
        <div class="section-heading split"><div><span class="eyebrow">Complete learning experience</span><h2>More than a collection of coding pages.</h2></div><p>CodeMwana combines lessons, practical work, assessment, progress tracking and teacher guidance in one responsive application.</p></div>
...
<section class="section final-cta"><div class="container"><div class="cta-panel"><div><span class="eyebrow light"><?= $registrationOpen ? 'Begin with your own account' : 'Continue your learning' ?></span><h2>Keep your learning progress together.</h2><p><?= $registrationOpen ? 'Create a learner account and keep lessons, scores, achievements and projects connected across sessions.' : 'New accounts are provided by the organisation. Existing learners can sign in to continue.' ?></p></div><a class="button button-light button-large" href="<?= e(url($registrationOpen ? 'register.php' : 'login.php')) ?>"><?= $registrationOpen ? 'Create learner account' : 'Sign in' ?><?= icon('arrow-right') ?></a></div></div></section>
