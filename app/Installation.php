...
final class Installation
{
    ...
    public static function state(bool $refresh = false): array
    {
        ...
        try {
            Database::connection();
            foreach (['users', 'site_settings'] as $table) {
    ...
    public static function enforce(): void
    {
        ...
        if ($state['database_error'] && $state['lock_exists']) {
            self::logTechnical('Installed application connection failure', $state['message']);
            self::renderFailure('Service temporarily unavailable', 'The learning platform cannot be reached at the moment. Please wait a short time and try again.', 503);
        }

        if ($state['setup_exists']) redirect('setup.php');

        self::logTechnical('Installation state could not be confirmed', $state['message']);
        self::renderFailure('Service not available', 'The learning platform is not ready for use. Please contact the organisation responsible for this service.', 503);
    }

    public static function migrationFailure(Throwable $exception): never
    {
        self::logTechnical('Application update failure', $exception->getMessage());
        self::renderFailure('Service temporarily unavailable', 'The platform is completing an update and cannot open this page yet. Please try again shortly.', 503);
    }

    private static function logTechnical(string $context, ?string $detail): void
    {
        error_log('CodeMwana: ' . $context . ($detail ? ' — ' . $detail : ''));
    }

    private static function renderFailure(string $title, string $message, int $status): never
    {
        http_response_code($status);
        header('Retry-After: 60');
        $safeTitle = htmlspecialchars($title, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
        $safeMessage = htmlspecialchars($message, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
        $homeUrl = htmlspecialchars(url('index.php'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
        echo '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>Service unavailable</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;padding:24px;background:#f4f5fa;color:#172033;font-family:system-ui,sans-serif}.card{width:min(100%,640px);padding:clamp(24px,5vw,44px);border:1px solid #e0e3ec;border-radius:24px;background:#fff;box-shadow:0 24px 80px rgba(27,31,55,.12)}.mark{display:grid;place-items:center;width:54px;height:54px;border-radius:16px;background:#efedff;color:#5b4bdb;font-weight:900}.eyebrow{margin:24px 0 8px;color:#5b4bdb;font-size:.72rem;font-weight:850;letter-spacing:.12em;text-transform:uppercase}h1{margin:0;font-size:clamp(1.8rem,5vw,3rem);line-height:1.08}p{color:#667085;line-height:1.7}.actions{display:flex;flex-wrap:wrap;gap:10px;margin-top:24px}a,button{display:inline-flex;align-items:center;justify-content:center;min-height:44px;padding:10px 16px;border:1px solid #5b4bdb;border-radius:12px;background:#5b4bdb;color:#fff;text-decoration:none;font-weight:800}button{background:#fff;color:#172033;border-color:#dfe2ea;cursor:pointer}</style></head><body><main class="card"><div class="mark">CM</div><p class="eyebrow">Platform notice</p><h1>' . $safeTitle . '</h1><p>' . $safeMessage . '</p><div class="actions"><button type="button" onclick="location.reload()">Try again</button><a href="' . $homeUrl . '">Return home</a></div></main></body></html>';
        exit;
    }
}
